...
int somme(const BoolMatrix &BM, int taille){
...
int compute_gradual_support(const BoolMatrix &BM, vector<int> *path_length){
  ...
  if(choose == 1){
	...
	//printf("Bonjour");
	  ...
	  //if(somme(BM, BM.get_size()) < threshold * (threshold - 1)/2){
		//sizing_cond = sizing_cond + 1;
		//return 0;
...
int main(int argc, char **argv){
  ...
  FILE *fiche, *fiche2, *fiche3;
  //fiche4 = fopen("le_temps.txt", "a");
  fiche = fopen("test_dat_freq_pattern_numb.txt", "a"); 
  fiche2 = fopen("test_dat_pattern_numb.txt", "a");
  fiche3 = fopen("ignored_patterns.txt", "a");
  fprintf(fiche, "%d,",num_pattern);
  printf("%d", sizing);
  fprintf(fiche2,"%d,",sizing);
  sizing = 0;
  if(choose == 0) fprintf(fiche3,"%d,",sizing_cond);
  //fprintf(fiche4,"%d,", time2-time1);
  fclose(fiche);
  fclose(fiche2);
  fclose(fiche3);
  //fclose(fiche4);
